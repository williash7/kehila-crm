...
import {
  ...
  NavItemId, normalizeBottomNavOrder, normalizeBottomNavPrimary,
} from '../lib/navigation';
        ...
        {order.map((id, index) => {
          ...
          return (
            <div key={id} className="flex items-center gap-2 border border-[#EDE6D6] rounded-xl px-2.5 py-2 bg-[#FAF6EE]">
              ...
              <button onClick={() => togglePrimary(id)} aria-pressed={shown} className={`min-w-[92px] rounded-lg px-2.5 py-1.5 text-[11px] font-bold flex items-center justify-center gap-1.5 ${shown ? 'bg-emerald-100 text-emerald-800' : 'bg-white border border-[#EDE6D6] text-gray-500'}`}>
