...
import { getCustomHols, apiPost } from '../lib/api';
...
export function GlobalAIImportModal({ onClose }: { onClose: () => void }) {
  const {
    holidays, eventsData, homeVisits, donors, holidayExtras,
    updateHolidayExtras, updateEventsData, updateHomeVisitRoundMeta, refresh,
  } = useAppStore();
  ...
  const [rows, setRows] = useState<Record<string, SheetRow[]>>({ contacts: [], donations: [], standingOrders: [] });
  ...
  const buildPrompt = () => {
    ...
    parts.push('\nהאפליקציה קולטת ארבעה סוגי מידע. כלול רק את הסוגים שבאמת קיימים במה שאצרף:');
    parts.push('1. אנשי קשר · 2. תרומות · 3. הוראות קבע · 4. משימות');
    ...
    parts.push('· לכל משימה כתוב שיוך (targetKind) אחד מתוך: "holiday", "event", "homeVisit", "contact" (עם שם האדם ב-targetLabel), "standalone".');
  ...
  const parsePasted = () => {
    ...
    // ── שורות לגיליון ────────────────────────────────────────────────────
    // מסננים לפי השדה שבלעדיו השורה חסרת משמעות, כדי ששורה ריקה שה-AI
    ...
    const total = resolvedItems.length + nextRows.contacts.length + nextRows.donations.length + nextRows.standingOrders.length;
    if (total === 0) {
  ...
  const sheetRowCount = rows.contacts.length + rows.donations.length + rows.standingOrders.length;
  const totalCount = items.length + sheetRowCount;
        ...
        {step === 'prompt' && (
          <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3">
            <p className="text-[11px] text-gray-500 leading-relaxed">
              ...
              והחזר הנה את ה-JSON שקיבלת. אפשר לייבא אנשי קשר, תרומות, הוראות קבע ומשימות —
              יחד או בנפרד. לפני השמירה תראה בדיוק מה עומד להיכנס.
            </p>
            {!prompt ? (
        ...
        {step === 'review' && (
          <>
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3 mb-3">
              ...
              {(['contacts', 'donations', 'standingOrders'] as const).map(section => rows[section].length > 0 && (
                <div key={section}>
